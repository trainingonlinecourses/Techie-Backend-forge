...
1. `@Mock OrderRepository repo;` — a fake collaborator
2. `@InjectMocks OrderService service;` — service gets the mocks injected
3. `void setUp() { MockitoAnnotations.openMocks(this); }` — or @ExtendWith(MockitoExtension.class)
4. `verify(repo).save(o);` — the collaborator was called with o
5. `verify(payments, never()).charge(any());` — and nothing else happened
...
2. `when(clock.instant()).thenReturn(Instant.parse("2026-08-17T10:00:00Z"));` — time travel!
3. Stub exceptions:
4. Stub by argument matchers (ALL args must be matchers):
5. Chain responses:
...
when(clock.instant()).thenReturn(Instant.parse("2026-08-17T10:00:00Z"));
