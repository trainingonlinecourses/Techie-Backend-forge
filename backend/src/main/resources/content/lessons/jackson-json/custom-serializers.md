...
1. Parse into a tree:
2. Traverse:
3. `String name = root.get("name").asText();` — Ada
4. `String firstTag = root.get("tags").get(0).asText();` — a
5. `int version = root.path("meta").path("v").asInt();` — 1 (path = missing-safe)
6. Modify and re-serialize:
...
int version = root.path("meta").path("v").asInt();
