...
1. @JsonProperty: the Java field's JSON name. The Java name is id; the JSON key is "user_id".
2. @JsonIgnore: never serialize or deserialize this field. (A password hash, a transient cache — anything internal.)
3. @JsonAlias: EXTRA accepted input names (output always uses the field name). The API accepts "mail" as well as "email" — forward compatibility.
4. @JsonFormat: how a temporal type renders.
5. @JsonInclude: when to OMIT a field from output. NON_NULL: skip null fields entirely ("active": null disappears).
6. getters/setters...
...
1. Default: enums serialize as their NAME: "PENDING". @JsonFormat on the FIELD: string enum per-field:
2. Or @JsonValue on the enum itself — a custom representation:
3. `@JsonValue` — serializes as "pending" (the code)
4. + @JsonCreator for the reverse direction, or @JsonEnumDefaultValue
