...
1. `Supplier<List<String>> listFactory = ArrayList::new;` — () -> new ArrayList<>()
2. `Function<Integer, List<String>> sized   = ArrayList::new;` — n -> new ArrayList<>(n)
3. The classic map-to-object pattern:
4. `.map(User::new)` — compiler picks whichever User constructor matches
5. `.toList();` — ...here: User(String email) exists → chosen
