...
1. Common fields and methods
2. Each permitted class must be one of: 1. final — cannot be extended further. 2. sealed — has its own permits clause. 3. non-sealed — anyone can extend (default)
...
1. ↑ sealed = this class controls who extends it. ↑ permits = explicit list of allowed subclasses. ↑ Only these THREE classes can extend Payment. ↑ Any other class trying to extend Payment → compile error
2. ↑ Common field — all payment types share this
3. ↑ protected constructor — only subclasses can call this. ↑ Not public — external code cannot create Payment directly
4. ↑ Public accessor — all payment types share this method
5. Each permitted subclass MUST be one of: final, sealed, or non-sealed
6. ↑ final = this class cannot be extended further. ↑ It's a leaf in the type hierarchy
7. `super(txId);` — ↑ Call parent constructor
8. Accessor methods...
9. Now pattern matching is exhaustive:
10. ↑ No default needed — compiler knows ALL cases. ↑ If you add a new Payment subtype, this code won't compile. ↑ Until you handle the new case — compile-time safety!
